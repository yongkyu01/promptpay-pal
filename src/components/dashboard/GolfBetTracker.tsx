    ...
    <div className="rounded-2xl border border-border bg-card p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-sm font-semibold text-foreground">
          {lang === "th" ? "สถิติพนันกอล์ฟ" : "Golf Bet Tracker"}
        </h2>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-xl bg-green-500/10 p-2.5 text-center">
          <p className="text-[10px] text-muted-foreground">{lang === "th" ? "ชนะ" : "Won"}</p>
        ...
        <div className="rounded-xl bg-destructive/10 p-2.5 text-center">
          <p className="text-[10px] text-muted-foreground">{lang === "th" ? "แพ้" : "Lost"}</p>
        ...
        <div className={`rounded-xl p-2.5 text-center ${netProfit >= 0 ? "bg-green-500/10" : "bg-destructive/10"}`}>
          <p className="text-[10px] text-muted-foreground">{lang === "th" ? "สุทธิ" : "Net"}</p>
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-muted-foreground">{winRate}% {lang === "th" ? "ชนะ" : "Win"}</span>
      </div>
        ...
        <div className="h-32">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} barSize={20}>
              ...
              <Tooltip
                formatter={(value: number) => [`฿${Math.abs(value).toLocaleString()}`, value >= 0 ? (lang === "th" ? "ชนะ" : "Won") : (lang === "th" ? "แพ้" : "Lost")]}
