...
export default function ExpenseDetailSheet({ expense, open, onOpenChange, lang, slipImageUrl }: Props) {
  ...
  const handleShareLine = () => {
    ...
    // LINE share URL scheme — works on mobile (opens LINE app) and desktop (opens LINE web)
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-2xl px-5 pb-8">
        <SheetHeader className="mb-4">
          <SheetTitle className="text-base">{lang === "th" ? "รายละเอียดรายจ่าย" : "Expense Detail"}</SheetTitle>
        </SheetHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 rounded-xl bg-secondary p-3">
            <div>
              <p className="text-[10px] text-muted-foreground">{lang === "th" ? "วันที่" : "Date"}</p>
              <p className="text-xs font-medium text-foreground">{expense.date}</p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground">{lang === "th" ? "เวลา" : "Time"}</p>
