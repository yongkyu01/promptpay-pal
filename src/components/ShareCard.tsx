...
export default function ShareCardButton({ totalSpending, slipCount, topCategory, refNo }: ShareCardProps) {
  ...
  const generateCard = () => {
    ...
    ctx.fillText(t("spendingSummary", lang), w / 2, 130);
    ...
    ctx.fillText(t("totalSpendingFull", lang), w / 2, cardY + 45);
    ...
    ctx.font = "500 14px Inter, sans-serif";
    ctx.fillText(`${slipCount} ${t("transactionsCount", lang)}`, w / 2, cardY + 140);
    ...
    if (topCategory) {
      ...
      ctx.fillText(t("topCategory", lang), w / 2, catY + 35);
    ...
    ctx.fillStyle = "rgba(255,255,255,0.5)";
    ctx.font = "500 13px Inter, sans-serif";
    ctx.fillText(t("fortuneScore", lang), w / 2, fY + 35);
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-sm p-4" onClick={() => setOpen(false)}>
          <div className="relative max-w-sm w-full animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <img src={canvasRef.current?.toDataURL()} alt="Share card" className="w-full rounded-2xl shadow-2xl" />
